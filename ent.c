...
typedef struct {
    char *digits; /* dígitos decimales 0..9, guardados en orden invertido */
    int   len;    /* cantidad de dígitos significativos */
    int   sign;   /* 1 = positivo, -1 = negativo (siempre 1 en esta versión) */
} Bigint;
...
Bigint *bigint_new(const char *text) {
    ...
    b->digits = malloc(n + 1); /* +1 por si el resultado de una suma necesita un dígito más */
    if (b->digits == NULL) {
        fprintf(stderr, "Error: sin memoria\n");
        free(b);
        return NULL;
    ...
    b->sign = 1; /* siempre positivo en esta versión */
    ...
    int j = 0;
    for (int i = n - 1; i >= 0; i--) {
        b->digits[j++] = text[i] - '0'; /* convertimos char '5' al número 5 */
    }
    b->len = n;
 ...
 * Esto es muy útil en la suma para no salirse del array.
 */
...
int main(int argc, char *argv[]) {
    ...
    Bigint *acumulador = bigint_new(argv[2]);
